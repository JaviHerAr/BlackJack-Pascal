...
TYPE
	Tvalor = 0..11; (*Representa los valores de cada carta*)
		      	(*El cero es para inicializar las bazas*)

	Ttam_baraja = 1..MaxCartas;(*Representa el tamaño de una baraja*)
	
	Tcartas	= Record
			carta:String[4];
			valor:Tvalor;
		  end;(*Representa cada una de las cartas de la baraja*)
...
Var
	i:Integer;(*Indice para recorrer toda la baraja*)
...
Begin
	...
	For i:=1 to MaxCartas do
		Begin
			...
			Case card of
				1:Begin
					baraja[i].carta:='[A]'; baraja[i].valor:= 11;
				  end;
				2..10:Begin
					str(card,cadena);(*Pasamos a cadena la carta para poder guardarla en la baraja*)
					baraja[i].carta:='['+cadena+']'; baraja[i].valor:= card;
				      end;
				11:Begin
					baraja[i].carta:='[J]'; baraja[i].valor:=10;
				   end;
				12:Begin
					baraja[i].carta:='[Q]'; baraja[i].valor:=10;
				   end;
				13:Begin
					baraja[i].carta:='[K]'; baraja[i].valor:=10;
				   end;
...
Var
	i:Integer;(*Indice para recorrer toda la baraja*)
	carta_aux:Tcartas;(*Variable auxiliar para poder hacer el intercambio de cartas*)
...
Begin
	...
	For i:=1 to MaxCartas do
		Begin
			...
			carta_aux.carta:= baraja[posic].carta; carta_aux.valor:= baraja[posic].valor;
			baraja[posic].carta:= baraja[i].carta; baraja[posic].valor:= baraja[i].valor;
			baraja[i].carta:= carta_aux.carta; baraja[i].valor:= carta_aux.valor;
		end;
...
Begin
	...
	Writeln(output,'');
	Writeln(output,'                         ------------------------');
	Writeln(output,'                         |REGLAS E INSTRUCCIONES|');
	Writeln(output,'                         ------------------------');
	Writeln(output,'');
	Writeln(output,'- El BlackJack se juega con una baraja de 52 cartas, con 4 palos,'); 
        Writeln(output,'  cada palo consta de un ás, 3 figuras (J,Q,K) y cartas numeradas del 2 al 10.');
	Writeln(output,'  Las cartas numeradas tienen un valor igual a su número, las figuras cuentan '); 
	Writeln(output,'  como 10 y los ases como 1 o 11, según le convenga al jugador.');
	Writeln(output,'');
	Writeln(output,'- El propósito del juego es conseguir una serie de cartas cuyo valor total sea');
 	Writeln(output,'  el máximo posible pero sin superar el valor 21 (que se denomina BlackJack y ');
	Writeln(output,'  es la mejor jugada).');
	Writeln(output,'');
	Writeln(output,'- El desarrollo de una partida del juego es el siguiente: ');
	Writeln(output,'');
	Writeln(output,'        1) APUESTA INICIAL: Siempre de 2 euros.');
	Writeln(output,'        2) REPARTO INICIAL: Dos cartas al jugador y una al casino.');
	Writeln(output,'        3) SERIE DEL JUGADOR: El jugador va pidiendo nuevas cartas hasta que '); 
	Writeln(output,'           se plante, obtenga un blackjack (21), o se pase (supere el valor 21).'); 
	Writeln(output,'           Al pedir una nueva carta tiene la opción de doblar la apuesta.');
	Writeln(output,'');
	Writeln(output,'         Si el jugador ha obtenido un blackjack o se ha pasado se omite el paso ');
	Writeln(output,'         siguiente (el casino no pide cartas).');
	Writeln(output,'');
	Writeln(output,'        4) SERIE DEL CASINO: Independientemente del valor obtenido por el jugador, ');
	Writeln(output,'           el casino va pidiendo nuevas cartas hasta que sumen un valor >= a 17.');
	Writeln(output,'        5) EVALUCACION DEL RESULTADO:');
	Writeln(output,'            -Si el jugador tiene blackjack gana y el casino le paga 3/2 de la apuesta.');
	Writeln(output,'            -Si el jugador se ha pasado pierde.');
	Writeln(output,'            -Si el jugador no se ha pasado y el casino sí, el jugador gana.');
	Writeln(output,'            -Si ambos no se han pasado, gana el que tenga una baza de valor mayor,'); 
	Writeln(output,'             o bien el casino en caso de empate.');
	Writeln(output,'');Writeln(output,'');
	Write(output,'¿ Desea [J]ugar o [S]alir ?: [J/S] ');
	Repeat
		opc:= Readkey;
...
Begin
	Writeln(output,'Partida: ',par,'| Apuesta: ',apu,'| Balance: ',bal);
end;(*balance_global*)
...
Var
	pos_baza:1..11;(*Contiene el indice de la baza donde colocaremos la carta robada*)
	i:Integer;(*Indice*)
Begin
	pos_baza:=1;
	...
	baza[pos_baza].carta:= baraja[card].carta; baza[pos_baza].valor:= baraja[card].valor;
	(*Pasamos la carta al monton de retiradas*)
	retiradas[card].carta:= baraja[card].carta; retiradas[card].valor:= baraja[card].valor;
	(*Eliminamos la carta de la baraja*)
	baraja[card].carta:='0'; baraja[card].valor:= 0;
	...
	If (card = 1) then 
		Begin
			...
			baraja:=retiradas; barajar(baraja);
...
VAR
	monton_reparto,monton_retiradas: Tbaraja; (*Monton de cartas pendientes de repartir / Monton de cartas ya repartidas*)
	cartas_jugador,cartas_casino: Tbaza; (*Almacena las cartas que va recibiendo el jugador y el casino*)
	partida,apuesta,balance:Integer;(*Contador de partidas / Contador de apuestas / Balance de ganancias y perdidas del jugador*)
	opcion:Char;(*Almacena las elecciones hechas por el jugador a lo largo de la partida *)
	apuntador_carta:Ttam_baraja;(*Contiene el indice de la carta que queremos robar*)
	baza_casino,baza_jugador:Integer;(*Contiene la baza en cada momento del casino y del jugador respectivamente*)
	serie_casino:Boolean;(*Indica si se ejecuta o no la serie del casino*)	
	ganada:Boolean;	(*Indica si el jugador ganó la partida o no*)
	t:integer;(*Indice*)
	enter:Char;(*Almacena la tecla enter cuando la pulsa el jugador*)
...
BEGIN
	...
	Writeln(output,'');
	Writeln(output,'                     ---------------------------------------------');
	Writeln(output,'                     | *** [21] ***    BLACKJACK    *** [21] *** |');
	Writeln(output,'                     ---------------------------------------------');
	Writeln(output,'');Writeln(output,'');
	Writeln(output,'                                 1- Reglas e instrucciones');
	Writeln(output,'                                 2- Jugar');
	Writeln(output,'                                 3- Salir');
	Writeln(output,'');Writeln(output,'');
	Write(output,'Escoja una opcion: ');
	...
	If (opcion = '2') or (opcion = 'j') then 
		Begin
			...
			crea_baraja(monton_reparto); barajar(monton_reparto);
			...
			Repeat
				...
				inicializar_baza(cartas_jugador); inicializar_baza(cartas_casino);
				For t:=1 to 2 do
				...
				retardo_cas:= True; retardo_jug:= True;(*Activamos los retardos al mostrar las cartas para el reparto inicial*)
				
				(*SERIE DEL JUGADOR*)
				Repeat
					(*Actualizamos las bazas del casino y el jugador*)
					baza_casino:=0; baza_jugador:=0;
					For t:=1 to 11 do
						Begin
					...
					retardo_cas:= False; retardo_jug:= True;						
					(*Evaluamos la baza obtenida*)
					If (baza_jugador >= 21) then
						Begin
							If (baza_jugador = 21) then
								Begin
									Write(output,'BLACKJACK! (Pulse ENTER)');
									Repeat
										enter:=Readkey;
									until (enter = #13);
									Writeln(output,'');	
									opcion:= 'p'; serie_casino:= False; ganada:=True;
									(*Incrementamos el balance en 3/2 de la apuesta actual*)
									apuesta:= ((apuesta div 2)*3); balance:= (balance + apuesta);
								end
							else
								Begin
									Write(output,'SE PASO! (Pulse ENTER)');
									Repeat
										enter:=Readkey;
									until (enter = #13);
									Writeln(output,'');
									opcion:= 'p'; serie_casino:= False; ganada:=False;
						...
						Begin
							...
							Write(output,'[O]tra carta, [P]lantarse, [D]oblar: [O/P/D] ');
							Repeat
								opcion:=Readkey;
				...
				While (serie_casino) do
					Begin
						...
						retardo_cas:= True; retardo_jug:= False;
						...
						If (baza_casino >= 17) then
							Begin
								If (baza_casino <= 21) then
									Begin
										Writeln(output,'Casino se planta'); Delay(1500);
									end
								else
									Begin
										Writeln(output,'Casino se paso'); Delay(1500);
										ganada:=True;
							...
							Begin
								Writeln(output,'Casino pide otra carta'); Delay(1500);
								serie_casino:= True;
							end;		
				...
				If (baza_jugador > baza_casino) and (baza_jugador < 21) then
					Begin
						ganada:=True; balance:= (balance + apuesta);
					end;

				If (baza_casino > baza_jugador) and (baza_casino <= 21) then
					Begin
						ganada:=False; balance:= (balance - apuesta); 
					end;
				...
				If ganada then
					Begin
						Writeln(output,'');
						Write(output,'GANASTE (+',apuesta,')   '); Delay(1000);
					end
				else
					Begin
						Writeln(output,'');
						Write(output,'PERDISTE (-',apuesta,')   '); Delay(1000);
					end;

				(*MENSAJE DE FINAL DE PARTIDA*)
				Write(output,'Otra ronda? [S/N]');
				Repeat 
					opcion:=Readkey;
